...
Размещается на Streamlit Community Cloud (бесплатно, без программирования).

Учётные данные бота задаются в настройках приложения (Secrets), НЕ в коде:
...
DATE_FIELD_NAME = ""              # если знаешь точное имя поля с датой — впиши
PAGE_SIZE = 1000
DATE_TYPES = ("date", "due_date", "due_date_time")
ACCENT = "#008C8C"
...
def pick_date_field(fields):
    by_name = {_norm(f.get("name")): f for f in fields}
    if DATE_FIELD_NAME and by_name.get(_norm(DATE_FIELD_NAME)):
        return by_name[_norm(DATE_FIELD_NAME)]
    date_fields = [f for f in fields if f.get("type") in DATE_TYPES]
    for f in date_fields:
        if "дата" in _norm(f.get("name")) or "платеж" in _norm(f.get("name")):
            return f
...
def load_report(login, key, d_from, d_to, only_income):
    ...
    date_name = date_field.get("name") if date_field else None
    ...
    rows, cursor = [], None
    while True:
        params = {"include_archived": "y", "sort": "id", "item_count": str(PAGE_SIZE)}
        if date_field:
            params[f"fld{date_field['id']}"] = f"gt{d_from.isoformat()},lt{d_to.isoformat()}"
    ...
    meta = {
        ...
        "fields": [(f.get("name"), f.get("type")) for f in fields],
    }
    return df, meta
...
def main():
    ...
    st.markdown(
        f"<h1 style='margin-bottom:0'>Отчёт по платежам</h1>"
        f"<p style='color:gray;margin-top:4px'>Pyrus · форма {FORM_ID}</p>",
    ...
    with st.sidebar:
        ...
        period = st.date_input("Период", value=(d_from, d_to), format="DD.MM.YYYY")
        only_income = st.checkbox(f"Только «{TYPE_TARGET_VALUE}»", value=True)
        if not login or not key:
            st.info("Учётные данные бота не заданы в настройках приложения (Secrets).")
    ...
    if not (run or st.session_state.get("loaded")):
        st.info("Выберите период слева и нажмите «Построить отчёт».")
        return
    ...
    st.caption(f"Период: {d_from:%d.%m.%Y} – {d_to:%d.%m.%Y}")
    ...
    # график по дням (если есть дата и сумма)
    if meta["date_name"] and meta["money_cols"]:
    ...
    # таблица с форматированием сумм
    ...
    with st.expander("Поля формы (для проверки названий)"):
        st.table(pd.DataFrame(meta["fields"], columns=["Поле", "Тип"]))
